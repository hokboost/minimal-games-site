...
class StreamerGameRepository {
    ...
    async lockAccounts(client, usernames) {
        ...
        const result = await client.query(`
            ...
            WHERE account.username=ANY($1::TEXT[]) ORDER BY account.id FOR UPDATE OF account
        `, [unique]);
    ...
    async findActiveCreatorRun(client, creatorUserId, gameId) {
        const result = await client.query(`SELECT id,revision FROM streamer_game_runs
            WHERE creator_user_id=$1 AND game_id=$2 AND status='active' FOR UPDATE`, [creatorUserId, gameId]);
    ...
    async collectionState(username) {
        const items = await this.pool.query(`SELECT item.item_key,item.acquired_at FROM streamer_game_collection_items item
            JOIN users account ON account.id=item.user_id WHERE account.username=$1 ORDER BY item.acquired_at,item.item_key`, [username]);
        const slots = await this.pool.query(`SELECT slot.slot_index,slot.item_key,slot.revision FROM streamer_game_room_slots slot
            JOIN users account ON account.id=slot.user_id WHERE account.username=$1 ORDER BY slot.slot_index`, [username]);
