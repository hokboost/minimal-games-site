...
管理员登录和高风险操作要求当前管理员会话、CSRF、限流、密码与 TOTP。`ADMIN_TOTP_SECRETS_JSON` 必须按管理员用户名配置独立密钥；已用登录 counter 不可重放。成功和失败结果都会写入 `admin_audit_log`。生产环境不支持管理员 IP 白名单，因为移动网络地址不稳定；管理权限依赖身份、step-up 和审计。
