...
管理员登录要求密码；登录后的后台操作要求当前管理员会话、CSRF 和严格限流，成功和失败结果都会写入 `admin_audit_log`，但不再要求二次密码或验证码。管理员 TOTP 不再是配置项或登录条件，因此这是明确的单因子管理模式：管理员必须使用独立高熵密码并妥善保管。生产环境不支持管理员 IP 白名单，因为移动网络地址不稳定；管理权限依赖登录身份、会话保护、限流和审计。
