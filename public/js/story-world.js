...
(() => {
    ...
    function render() {
        ...
        if (!story) { const intro = document.createElement('p'); intro.textContent = lang === 'zh' ? '第一季已经准备好。你可以按自己的节奏开始。' : 'Season One is ready whenever you are.'; stage.append(intro, button(lang === 'zh' ? '开始第一季' : 'Start Season One', 'start')); return; }
        ...
        else if (node.action === 'finish') stage.append(button(lang === 'zh' ? '完成第一季' : 'Complete Season One', 'finish'));
    ...
    stage.addEventListener('click', async (event) => {
        ...
        try {
            ...
            if (action === 'start' || action === 'replay') response = await post('/api/story/runs/start', { replay: action === 'replay' });
            ...
            pendingChoice = null; model = { ...response, hasRun: true }; if (response.outcome) message.textContent = response.outcome; render();
