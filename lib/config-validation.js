...
const { parseAdminTotpSecrets } = require('./admin-mfa');
...
function validateServerEnvironment() {
    ...
    if (process.env.NODE_ENV === 'production') {
        ...
        parseAdminTotpSecrets(requireValue('ADMIN_TOTP_SECRETS_JSON', 2));
    ...
    if (process.env.ADMIN_TOTP_SECRETS_JSON) {
        parseAdminTotpSecrets(process.env.ADMIN_TOTP_SECRETS_JSON);
    }
}
