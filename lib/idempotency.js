...
const { PRE_BUSINESS_CAPACITY_REJECTION } = require('./concurrency-guard');
...
const PENDING_RESPONSE = Object.freeze({
    success: false,
...
function createIdempotencyMiddleware({
    ...
    retryableCapacityRoutes = [],
    hashSecret = ''
}) {
    const protectedPaths = new Set(paths);
    const retryableCapacityRouteKeys = new Set(retryableCapacityRoutes);
    ...
    return async function requireIdempotency(req, res, next) {
        ...
        try {
            ...
            if (inserted.rows.length === 0) {
                ...
                return res.status(409).json(PENDING_RESPONSE);
            }
            ...
            const finalizeResponse = (sender, args, body) => {
                ...
                const retryableCapacityRejection = !transactionDenial
                    && responseStatus === 503
                    && req[PRE_BUSINESS_CAPACITY_REJECTION] === true
                    && retryableCapacityRouteKeys.has(routeKey);
                res.set('Idempotency-Status', retryableCapacityRejection ? 'retryable' : 'created');
                const finalize = retryableCapacityRejection
                    ? retryQuery(pool, `
                        DELETE FROM idempotency_keys
                        WHERE username = $1 AND idempotency_key = $2
                          AND request_hash = $3 AND status = 'pending'
                        RETURNING id
                    `, [username, key, requestHash]).then(async (deleted) => {
                        if (deleted.rows.length > 0) return { status: 'retryable' };
                        const current = await retryQuery(pool, `
                            SELECT status, response_status, response_body
                            FROM idempotency_keys
                            WHERE username = $1 AND idempotency_key = $2
                        `, [username, key]);
                        return current.rows[0] || { status: 'retryable' };
                    })
                    : responseStatus >= 500
                    ? retryQuery(pool, `
                        UPDATE idempotency_keys
